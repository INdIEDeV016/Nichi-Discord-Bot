...
var APPLICATION_COMMAND_TYPE = [
	'CHAT_INPUT',
	'USER',
	'MESSAGE'
]

var APPLICATION_COMMAND_OPTION_TYPE = [
	'SUB_COMMAND',
	'SUB_COMMAND_GROUP',
	'STRING',
	'INTEGER',
...
# Public Functions
func login(token: String, app_id: String = '') -> void:
	TOKEN = token
	...
	if _login_error == ERR_INVALID_PARAMETER:
		...
		var coroutine = self.login(TOKEN)
	else:
		match _login_error:
			ERR_UNAUTHORIZED:
...
func add_application_commands(p_options: Dictionary, guild_id: String = ""):
	"""
	p_options {
		'name': String,
...
#		assert(p_options.type.to_upper() in APPLICATION_COMMAND_TYPE, 'Invalid Type: status must be one of APPLICATION_COMMAND_TYPE')
	...
	if not p_options.options.empty():
		for option in p_options.options:
			...
			assert(option.type is int, 'Type has to be an Integer: %s' % option)
			assert(option.description is String, 'Description has to be a String: %s' % option)
			assert(option.description, 'Description can\'t be empty: %s' % option)
			assert(option.required is bool, 'Option has to be a Boolean value: %s' % option)
		if p_options.options.has('choices'):
			if not p_options.options.choices.empty():
				for choice in p_options.options.choices:
...
func set_presence(p_options: Dictionary) -> void:
	...
	assert(p_options is Dictionary, 'Invalid Type: options in set_presence must be a Dictionary')
	...
	if p_options.has('activity') and p_options.activity is Dictionary:
...
func _handle_events(dict: Dictionary) -> void:
	...
	if VERBOSE:
		prints("Event:", event_name)
	emit_signal("event", self, event_dict)
	match event_name:
		'HELLO':
			pass
...
# Helper functions
static func _jsonstring_to_dict(data: String) -> Dictionary:
